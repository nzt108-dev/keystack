...
# ---- live map (spec-live-map.md story 1) ---------------------------------------------------------
#
# Reads <proj_dir>/.flowmap/ artifacts generated by architect-portfolio's scripts/flowmap/* — this
# scanner NEVER generates or writes those files, only counts what's already there (read-model,
# same invariant as the specs/ table above). Prints "has_flowmap|crit|warn":
#   has_flowmap = 1 iff .flowmap/ directory exists (0 otherwise, and crit/warn are then 0 too).
#   crit        = ui-flow.json .verdict.crit (0 when ui-flow.json is absent — non-Next.js project,
#                 or the UI-layer scan hasn't been run yet; that's "no data", not zero issues found).
#   warn        = ui-flow.json .verdict.warn + architecture.json .issues length.
# Mapping note: architecture.json issues (ArchIssue = {message} only, NO severity field in the
# generator's contract — confirmed against architect-portfolio/scripts/flowmap/types.ts) are
# structural notes ("known deps not found in manifest"), never a broken click-path — they fold into
# warn, never crit. ui-flow.json already computes verdict.{crit,warn} itself (severity-tagged
# issues), so this scanner reads it as-is rather than re-deriving it from raw issue kinds.
# A malformed/missing jq, or unparsable JSON, degrades to 0/0 — never errors out the whole scan.
check_flowmap() {
  local proj_dir="$1" fm="$proj_dir/.flowmap" crit=0 warn=0 arch_issues=0
  ...
  if ! command -v jq >/dev/null 2>&1; then
    echo "  ! keystack-scan: jq not found — has_flowmap=1 but map_crit/map_warn forced to 0" >&2
    echo "1|0|0"
    return
  fi

  if [ -f "$fm/ui-flow.json" ]; then
    crit=$(jq -r '.verdict.crit // 0' "$fm/ui-flow.json" 2>/dev/null)
    warn=$(jq -r '.verdict.warn // 0' "$fm/ui-flow.json" 2>/dev/null)
  fi
  [ -n "$crit" ] && [ "$crit" -eq "$crit" ] 2>/dev/null || crit=0
  [ -n "$warn" ] && [ "$warn" -eq "$warn" ] 2>/dev/null || warn=0

  if [ -f "$fm/architecture.json" ]; then
    arch_issues=$(jq -r '.issues | length' "$fm/architecture.json" 2>/dev/null)
  fi
  [ -n "$arch_issues" ] && [ "$arch_issues" -eq "$arch_issues" ] 2>/dev/null || arch_issues=0

  echo "1|$crit|$((warn + arch_issues))"
}
...
scan_one_project() {
  ...
  local flowmap_row has_fm map_crit map_warn
  flowmap_row=$(check_flowmap "$proj_dir")
  IFS='|' read -r has_fm map_crit map_warn <<< "$flowmap_row"
  ...
  echo "  ok  $slug: arch=$has_arch inv=$has_inv design=$has_design ci=$has_ci_flag tests=$tcount green=$tgreen score=$score flowmap=$has_fm crit=$map_crit warn=$map_warn"
  return 0
}

...
CREATE TABLE IF NOT EXISTS projects (
  ...
  -- spec-live-map.md story 1 (Фаза A): live map read-model, filled by keystack-scan.sh from
  -- <local_path>/.flowmap/ artifacts (generated by architect-portfolio's flowmap scripts — keystack
  -- never generates these, only reads them). Card badge only; /map/:slug itself re-reads live.
  has_flowmap      INTEGER DEFAULT 0,         -- 0/1 — <local_path>/.flowmap/ exists
  map_crit         INTEGER DEFAULT 0,         -- ui-flow.json verdict.crit (0 if ui-flow.json absent)
  map_warn         INTEGER DEFAULT 0          -- ui-flow.json verdict.warn + architecture.json issues.length
);
